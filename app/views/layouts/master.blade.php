         ...
         <ul class="nav navbar-nav navbar-right ">
              @if(Auth::check())
                   <li class=" @if( $page == "dashboard/notifications" || $page == "dashboard/profile" || $page == "dashboard/editProfile" || $page == "dashboard/addProject") {{'active'}} @endif">
                        <a href="{{URL::to('dashboard/notifications')}}">{{Auth::user()->profile->firstName ," ",Auth::user()->profile->lastName}}@if(Auth::user()->notifications->count() > 0)<span class="label btn-green">{{Auth::user()->notifications->count()}}</span>@endif </a></li>
